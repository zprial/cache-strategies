...
如果缓存可用，则使用缓存响应尽快响应；如果没有缓存，则返回到 api 调用，然后使用 api 调用更新缓存。与某些过时而重新验证的实现不同，此策略将始终会调用 api，而不考虑缓存响应的时间。
...
一般不常用这个策略，因为平时默认就是 api 调用 返回数据。
